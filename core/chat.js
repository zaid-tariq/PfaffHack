class Chat {
    ...
    send_message(user1_name, user2_name, message, eventBroker){  
       ...
       eventBroker.request('get-user', {
            ...
            console.log(res);
            this.find_second_user(res["payload"]["userId"], user2_name, message, eventBroker);
        }).catch((err)=>console.log(err))
    }

    find_second_user(user1_id, user_2name, message,eventBroker){
    ...
    find_chat_room(user1_id, user2_id, message,eventBroker){
      //   {
      ...
      eventBroker.request('create-chat-global', {
                "participants": [ user1_id
                 ,user2_id]
            },
      ).then((res)=>{
          console.log(res);
          this.send_message_(user1_id, user2_id, res["payload"]["room"]["roomId"], message,eventBroker);
      }).catch((err)=>console.log(err))      
    }

    send_message_(user1_id, user2_id, roomId, message,eventBroker){
      
      console.log(roomId);
      //this.eventBroker.subscribe('send-chat-message', (event, topic) => console.log(event) /* do something when this event occurs */)
       
      eventBroker.request('send-chat-message', {
                "roomId":roomId,
                "sender":user1_id,
